...
class PCVRHyFormerRankingTrainer:
    ...
    def __init__(
        ...
        empty_cache_after_eval: bool = False,
        warmup_steps: int = 0,
        ...
        self.writer_interval: int = max(1, writer_interval)
        self.show_progress: bool = show_progress
        self.empty_cache_after_eval: bool = empty_cache_after_eval
        ...
        self.use_amp = self.device.startswith('cuda') and amp_mode in {'bf16', 'fp16'}
        self.amp_dtype = torch.bfloat16 if amp_mode == 'bf16' else torch.float16
        self.use_grad_scaler = self.use_amp and amp_mode == 'fp16'
        self.grad_scaler = torch.amp.GradScaler('cuda', enabled=self.use_grad_scaler)

        logging.info(f"PCVRHyFormerRankingTrainer loss_type={loss_type}, "
                     f"focal_alpha={focal_alpha}, focal_gamma={focal_gamma}, "
                     f"reinit_sparse_after_epoch={reinit_sparse_after_epoch}, "
                     f"pairwise_lambda={self.pairwise_lambda}, "
                     f"amp_mode={self.amp_mode}, grad_clip_norm={self.grad_clip_norm}, "
                     f"writer_interval={self.writer_interval}, show_progress={self.show_progress}, "
                     f"empty_cache_after_eval={self.empty_cache_after_eval}")
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                ...
                if self.writer and total_step % self.writer_interval == 0:
                    self.writer.add_scalar('Loss/train', loss, total_step)
                    if self.dense_scheduler is not None:
                        self.writer.add_scalar(
                            'LR/dense',
                            self.dense_scheduler.get_last_lr()[0],
                            total_step,
                        )
                ...
                if self.eval_every_n_steps > 0 and total_step % self.eval_every_n_steps == 0:
                    ...
                    self.model.train()
                    if self.empty_cache_after_eval and self.device.startswith('cuda'):
    ...
    def _make_model_input(self, device_batch: Dict[str, Any]) -> ModelInput:
        ...
        return ModelInput(
            ...
            meta_feats=device_batch['meta_feats'],
            seq_data=seq_data,
            seq_lens=seq_lens,
