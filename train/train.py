"""PCVRHyFormer flash-attn speed-optimized training entry point.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="PCVRHyFormer FlashAttn Speed Training")
    ...
    parser.add_argument('--train_prefetch_factor', type=int, default=4,
                        help='Prefetch factor for training DataLoader workers')
    parser.add_argument('--valid_num_workers', type=int, default=2,
                        help='Number of validation DataLoader workers')
    parser.add_argument('--valid_prefetch_factor', type=int, default=2,
                        help='Prefetch factor for validation DataLoader workers')
    ...
    parser.add_argument('--use_meta_query_conditioning', type=int, default=1,
                        help='Use v24 early meta conditioning in query generation '
                             '(1 = enabled, 0 = disabled)')
    parser.add_argument('--meta_query_gate_init', type=float, default=0.01,
                        help='Initial gate for the v24 early meta query path')
    ...
    parser.add_argument('--amp_mode', type=str, default='bf16',
                        choices=['off', 'auto', 'bf16', 'fp16'],
                        help='Mixed precision mode')
    parser.add_argument('--enable_tf32', type=int, default=1,
                        help='Enable TF32 matmul / cuDNN kernels when available')
    parser.add_argument('--cudnn_benchmark', type=int, default=1,
                        help='Enable cuDNN benchmark auto-tuning')
    ...
    parser.add_argument('--writer_interval', type=int, default=100,
                        help='Write TensorBoard scalars every N training steps')
    parser.add_argument('--show_progress', type=int, default=0,
                        help='Enable tqdm progress bars (1=yes, 0=no)')
    parser.add_argument('--empty_cache_after_eval', type=int, default=0,
                        help='Call torch.cuda.empty_cache() after evaluation')
...
def main() -> None:
    ...
    logging.info(f"Requested attention backend: {os.environ.get('TAAC_ATTN_BACKEND', 'sdpa')}")
    ...
    if torch.cuda.is_available():
        ...
        torch.backends.cudnn.benchmark = bool(args.cudnn_benchmark) and not bool(args.deterministic)
        torch.backends.cudnn.deterministic = bool(args.deterministic)

    logging.info(
        "Runtime speed config: "
        f"amp_mode={args.amp_mode}, enable_tf32={bool(args.enable_tf32)}, "
        f"cudnn_benchmark={bool(args.cudnn_benchmark)}, deterministic={bool(args.deterministic)}, "
        f"matmul_precision={args.matmul_precision}, train_prefetch_factor={args.train_prefetch_factor}, "
        f"valid_num_workers={args.valid_num_workers}, valid_prefetch_factor={args.valid_prefetch_factor}, "
        f"writer_interval={args.writer_interval}, show_progress={bool(args.show_progress)}, "
        f"empty_cache_after_eval={bool(args.empty_cache_after_eval)}"
    )
    ...
    model_args = {
        ...
        "user_int_entries": list(pcvr_dataset.user_int_schema.entries),
        "user_dense_entries": list(pcvr_dataset.user_dense_schema.entries),
        "meta_feature_dim": pcvr_dataset.meta_feature_dim,
        ...
        "use_meta_query_conditioning": bool(args.use_meta_query_conditioning),
        "meta_query_gate_init": args.meta_query_gate_init,
    }

    model = PCVRHyFormer(**model_args).to(args.device)
    train_config = vars(args).copy()
    train_config["meta_feature_dim"] = pcvr_dataset.meta_feature_dim
    ...
    logging.info(
        f"Meta features: dim={pcvr_dataset.meta_feature_dim}, "
        f"names={pcvr_dataset.meta_feature_names}"
    )
    ...
    trainer = PCVRHyFormerRankingTrainer(
        ...
        writer_interval=args.writer_interval,
        show_progress=bool(args.show_progress),
        empty_cache_after_eval=bool(args.empty_cache_after_eval),
