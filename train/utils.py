    ...
    def resync(self, model: nn.Module, param_ptrs: "set[int]") -> None:
        for name, param in model.named_parameters():
            if (
                ...
                and param.data_ptr() in param_ptrs
            ):
...
def set_seed(seed: int, deterministic: bool = True) -> None:
    ...
    Args:
        ...
        deterministic: When True, force cuDNN deterministic mode. When False,
            allow non-deterministic kernels so callers can enable
            ``cudnn.benchmark`` for higher throughput.
    """
    random.seed(seed)
