...
_META_USER_LIST_FIDS = (15, 64, 65, 66, 89, 90, 91)
_META_ITEM_LIST_FIDS = (11,)
_META_SCALAR_PRESENT_FIDS = {
    'item': (7, 8, 12, 81, 83, 84, 85),
}
_LOCAL_TIME_OFFSET_SECONDS = 8 * 3600
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        self._buf_meta = np.zeros((B, self.meta_feature_dim), dtype=np.float32)
        ...
        # Meta feature plans use raw columns before padding/clipping so the
        # model can recover information lost by fixed-length truncation.
        self._meta_user_list_plan = [
            (fid, self._col_idx.get(f'user_int_feats_{fid}'), self._user_int_dim_by_fid[fid])
            for fid in self._meta_user_list_fids
        ]
        self._meta_item_list_plan = [
            (fid, self._col_idx.get(f'item_int_feats_{fid}'), self._item_int_dim_by_fid[fid])
            for fid in self._meta_item_list_fids
        ]
        self._meta_item_scalar_plan = [
            (fid, self._col_idx.get(f'item_int_feats_{fid}'))
            for fid in self._meta_item_scalar_fids
        ]
    ...
    def _load_schema(self, schema_path: str, seq_max_lens: Dict[str, int]) -> None:
        ...
        self._init_meta_feature_layout()

    def _init_meta_feature_layout(self) -> None:
        """Define dense meta features derived from already-available columns."""
        names: List[str] = []
        self._meta_domain_feature_indices: Dict[str, Dict[str, int]] = {}
        for domain in self.seq_domains:
            idxs: Dict[str, int] = {}
            for key in (
                'orig_len_log',
                ...
                idxs[key] = len(names)
                names.append(f'{domain}_{key}')
            self._meta_domain_feature_indices[domain] = idxs

        self._meta_sentinel_specs: List[Tuple[str, int, int, int]] = []
        for domain in self.seq_domains:
            available = set(self.sideinfo_fids[domain])
            for fid in _META_SENTINEL_FIDS.get(domain, ()):
                if fid not in available:
                    continue
                zero_idx = len(names)
                names.append(f'{domain}_{fid}_zero_rate')
                neg_idx = len(names)
                names.append(f'{domain}_{fid}_neg_rate')
                self._meta_sentinel_specs.append((domain, fid, zero_idx, neg_idx))

        self._meta_user_list_fids = [
            fid for fid in _META_USER_LIST_FIDS if fid in self._user_int_dim_by_fid
        ]
        self._meta_item_list_fids = [
            fid for fid in _META_ITEM_LIST_FIDS if fid in self._item_int_dim_by_fid
        ]
        self._meta_list_feature_indices: Dict[Tuple[str, int], Tuple[int, int, int]] = {}
        for group, fids in (
            ('user_int', self._meta_user_list_fids),
            ('item_int', self._meta_item_list_fids),
        ):
            for fid in fids:
                len_idx = len(names)
                names.append(f'{group}_{fid}_len_log')
                present_idx = len(names)
                names.append(f'{group}_{fid}_present')
                fill_idx = len(names)
                names.append(f'{group}_{fid}_fill_frac')
                self._meta_list_feature_indices[(group, fid)] = (len_idx, present_idx, fill_idx)

        self._meta_item_scalar_fids = [
            fid for fid in _META_SCALAR_PRESENT_FIDS['item']
            if fid in self._item_int_dim_by_fid
        ]
        self._meta_scalar_feature_indices: Dict[Tuple[str, int], int] = {}
        for fid in self._meta_item_scalar_fids:
            idx = len(names)
            names.append(f'item_int_{fid}_present')
            self._meta_scalar_feature_indices[('item_int', fid)] = idx

        self._meta_time_indices = {}
        for key in ('hour_sin', 'hour_cos', 'dow_sin', 'dow_cos'):
            self._meta_time_indices[key] = len(names)
            names.append(f'timestamp_{key}')

        self._meta_calendar_hour_indices: List[int] = []
        for hour in range(24):
            self._meta_calendar_hour_indices.append(len(names))
            names.append(f'timestamp_hour_{hour:02d}')

        self._meta_calendar_dow_indices: List[int] = []
        for dow in range(7):
            self._meta_calendar_dow_indices.append(len(names))
            names.append(f'timestamp_dow_{dow}')

        self._meta_calendar_hour_dow_indices: List[int] = []
        for dow in range(7):
            for hour in range(24):
                self._meta_calendar_hour_dow_indices.append(len(names))
                names.append(f'timestamp_dow_{dow}_hour_{hour:02d}')

        self.meta_feature_names = names
        self.meta_feature_dim = len(names)

    @staticmethod
    def _list_lengths(col: "pa.Array", batch_size: int) -> np.ndarray:
        """Return per-row list lengths for a PyArrow list column."""
        if col is None:
            return np.zeros(batch_size, dtype=np.float32)
        offs = col.offsets.to_numpy()
        return (offs[1:] - offs[:-1]).astype(np.float32, copy=False)

    @staticmethod
    def _log_scaled(values: np.ndarray, scale: float) -> np.ndarray:
        return (np.log1p(np.maximum(values, 0.0)) / scale).astype(np.float32)

    def _fill_time_meta(self, meta: np.ndarray, timestamps: np.ndarray) -> None:
        local_ts = timestamps.astype(np.int64) + _LOCAL_TIME_OFFSET_SECONDS
        hour_int = ((local_ts // 3600) % 24).astype(np.int64)
        dow_int = ((local_ts // 86400 + 3) % 7).astype(np.int64)
        hour = hour_int.astype(np.float32)
        dow = dow_int.astype(np.float32)
        meta[:, self._meta_time_indices['hour_sin']] = np.sin(2.0 * np.pi * hour / 24.0)
        meta[:, self._meta_time_indices['hour_cos']] = np.cos(2.0 * np.pi * hour / 24.0)
        meta[:, self._meta_time_indices['dow_sin']] = np.sin(2.0 * np.pi * dow / 7.0)
        meta[:, self._meta_time_indices['dow_cos']] = np.cos(2.0 * np.pi * dow / 7.0)
        rows = np.arange(timestamps.shape[0])
        meta[rows, np.take(self._meta_calendar_hour_indices, hour_int)] = 1.0
        meta[rows, np.take(self._meta_calendar_dow_indices, dow_int)] = 1.0
        meta[
            rows,
            np.take(self._meta_calendar_hour_dow_indices, dow_int * 24 + hour_int),
        ] = 1.0
    ...
    def _fill_list_meta(
        ...
        for fid, ci, dim in plan:
            if ci is None:
                continue
            len_idx, present_idx, fill_idx = self._meta_list_feature_indices[(group, fid)]
            lens = self._list_lengths(batch.column(ci), batch_size)
            meta[:, len_idx] = self._log_scaled(lens, 8.0)
            meta[:, present_idx] = (lens > 0).astype(np.float32)
            meta[:, fill_idx] = np.clip(lens / max(float(dim), 1.0), 0.0, 1.0)

    def _fill_scalar_presence_meta(
        self,
        meta: np.ndarray,
        batch: "pa.RecordBatch",
        batch_size: int,
    ) -> None:
        for fid, ci in self._meta_item_scalar_plan:
            if ci is None:
                continue
            idx = self._meta_scalar_feature_indices[('item_int', fid)]
            arr = batch.column(ci).fill_null(0).to_numpy(zero_copy_only=False).astype(np.int64)
            meta[:, idx] = (arr[:batch_size] > 0).astype(np.float32)

    def _fill_sequence_meta(
        self,
        meta: np.ndarray,
        domain: str,
        timestamps: np.ndarray,
        ts_col: Optional["pa.Array"],
        batch_size: int,
    ) -> None:
        idxs = self._meta_domain_feature_indices[domain]
        if ts_col is None:
            meta[:, idxs['empty']] = 1.0
            return

        offs = ts_col.offsets.to_numpy()
        vals = ts_col.values.to_numpy()
        ...
        for i in range(batch_size):
            ...
            orig_len = e - s
            if orig_len <= 0:
                meta[i, idxs['empty']] = 1.0
                continue

            seq_ts = vals[s:e]
            valid_ts = seq_ts[seq_ts > 0]
            meta[i, idxs['orig_len_log']] = np.log1p(orig_len) / 8.0
            meta[i, idxs['over_maxlen']] = 1.0 if orig_len > max_len else 0.0
            meta[i, idxs['trunc_frac']] = max(orig_len - max_len, 0.0) / max(float(orig_len), 1.0)
            if valid_ts.size > 0:
                latest = int(valid_ts.max())
                oldest = int(valid_ts.min())
                latest_age = max(int(timestamps[i]) - latest, 0)
                span = max(latest - oldest, 0)
                meta[i, idxs['latest_age_log']] = np.log1p(latest_age) / 20.0
                meta[i, idxs['span_log']] = np.log1p(span) / 20.0

    def _fill_sentinel_meta(
        self,
        meta: np.ndarray,
        sentinel_cols: Dict[Tuple[str, int], "pa.Array"],
        batch_size: int,
    ) -> None:
        for domain, fid, zero_idx, neg_idx in self._meta_sentinel_specs:
            col = sentinel_cols.get((domain, fid))
            if col is None:
                continue
            offs = col.offsets.to_numpy()
            ...
            for i in range(batch_size):
                ...
                row_vals = vals[s:e]
                denom = float(e - s)
                meta[i, zero_idx] = np.count_nonzero(row_vals == 0) / denom
                meta[i, neg_idx] = np.count_nonzero(row_vals < 0) / denom
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        meta = self._buf_meta[:B]
        meta[:] = 0.0
        self._fill_time_meta(meta, timestamps)
        self._fill_list_meta(meta, batch, self._meta_user_list_plan, 'user_int', B)
        self._fill_list_meta(meta, batch, self._meta_item_list_plan, 'item_int', B)
        self._fill_scalar_presence_meta(meta, batch, B)
        ...
        for domain in self.seq_domains:
            ...
            col_data = []
            sentinel_fids = set(_META_SENTINEL_FIDS.get(domain, ()))
            for ci, slot, vs, fid in side_plan:
                col = batch.column(ci)
                col_data.append((col.offsets.to_numpy(), col.values.to_numpy(), vs, ci))
                if fid in sentinel_fids:
                    sentinel_cols[(domain, fid)] = col
            ...
            self._fill_sequence_meta(meta, domain, timestamps, ts_col, B)
            self._fill_sentinel_meta(meta, sentinel_cols, B)

        result['meta_feats'] = torch.from_numpy(meta.copy())
        return result
...
def get_pcvr_data(
    ...
    **kwargs: Any,
    ...
    if num_workers > 0:
        ...
        _train_kw['prefetch_factor'] = train_prefetch_factor
    ...
    if valid_num_workers > 0:
        ...
        _valid_kw['prefetch_factor'] = valid_prefetch_factor
    ...
    logging.info(f"Loader runtime: train_workers={num_workers}, "
                 f"train_prefetch_factor={train_prefetch_factor if num_workers > 0 else 0}, "
                 f"valid_workers={valid_num_workers}, "
                 f"valid_prefetch_factor={valid_prefetch_factor if valid_num_workers > 0 else 0}, "
