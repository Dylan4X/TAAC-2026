...
def _resolve_attention_backend() -> str:
    requested = os.environ.get("TAAC_ATTN_BACKEND", "auto").strip().lower()
    if requested not in {"auto", "sdpa", "flash_attn"}:
        requested = "auto"
    ...
    if requested == "flash_attn":
        if _flash_attn_func is not None and torch.cuda.is_available():
            if not _ATTN_BACKEND_LOG_STATE["selected"]:
                logging.info("Attention backend selected: flash_attn (forced)")
                _ATTN_BACKEND_LOG_STATE["selected"] = True
            return "flash_attn"
        if not _ATTN_BACKEND_LOG_STATE["missing"]:
            logging.info("Attention backend requested flash_attn but it is unavailable; falling back to sdpa")
            _ATTN_BACKEND_LOG_STATE["missing"] = True
        return "sdpa"

    if _flash_attn_func is not None and torch.cuda.is_available():
        if not _ATTN_BACKEND_LOG_STATE["selected"]:
            logging.info("Attention backend selected: flash_attn (auto)")
            _ATTN_BACKEND_LOG_STATE["selected"] = True
        return "flash_attn"

    if not _ATTN_BACKEND_LOG_STATE["missing"]:
        logging.info("Attention backend selected: sdpa")
        _ATTN_BACKEND_LOG_STATE["missing"] = True
    return "sdpa"
            ...
            if use_flash:
                try:
                    ...
                    if fa_dtype not in (torch.float16, torch.bfloat16):
                        fa_dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
                    q_fa = Q.transpose(1, 2).contiguous().to(fa_dtype)
                    k_fa = K.transpose(1, 2).contiguous().to(fa_dtype)
                    v_fa = V.transpose(1, 2).contiguous().to(fa_dtype)
                    ...
                    if not _ATTN_BACKEND_LOG_STATE["fallback"]:
                        logging.info(f"flash_attn failed; falling back to sdpa. reason={exc!r}")
                        _ATTN_BACKEND_LOG_STATE["fallback"] = True
    ...
    def forward(
        ...
        Args:
            ...
            seq_padding_masks: List of (B, L_i) masks, length S. True
                indicates padding.
            context_token: Optional (B, D) context added to each sequence
                summary before query generation.
        ...
        for i in range(self.num_sequences):
            ...
            if context_token is not None:
                seq_pooled = seq_pooled + context_token.to(seq_pooled.dtype)
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        user_int_entries: Optional[List[Tuple[int, int, int]]] = None,
        user_dense_entries: Optional[List[Tuple[int, int, int]]] = None,
        meta_feature_dim: int = 0,
        meta_gate_init: float = 0.05,
        use_meta_query_conditioning: bool = True,
        meta_query_gate_init: float = 0.01,
        ...
        self.use_meta_features = self.meta_feature_dim > 0
        self.use_meta_query_conditioning = (
            self.use_meta_features and bool(use_meta_query_conditioning)
        )
        self._user_int_entries = list(user_int_entries or [])
        self._user_dense_entries = list(user_dense_entries or [])
        self._user_int_fid_to_idx = {
            fid: i for i, (fid, _, _) in enumerate(self._user_int_entries)
        }
        self._user_dense_fid_to_slice = {
            fid: (offset, length) for fid, offset, length in self._user_dense_entries
        }
        self._structured_user_dense_single_fids = (61, 87)
        self._structured_user_dense_pair_groups = (
            (62, 63, 64, 65, 66),
            (89, 90, 91),
        )
        ...
        self.use_structured_user_dense = False
        if self.has_user_dense:
            self.user_dense_proj = nn.Sequential(
                nn.Linear(user_dense_dim, d_model),
                nn.LayerNorm(d_model),
            )
            self.use_structured_user_dense = self._init_structured_user_dense_modules(d_model)
            if self.use_structured_user_dense:
                logging.info(
                    "Structured user dense tokenizer enabled: "
                    f"singles={list(self._structured_user_dense_single_fids)}, "
                    f"pairs={[list(g) for g in self._structured_user_dense_pair_groups]}"
                )
        ...
        if self.use_meta_features:
            self.meta_proj = nn.Sequential(
                nn.Linear(self.meta_feature_dim, d_model),
                nn.LayerNorm(d_model),
                nn.SiLU(),
                nn.Dropout(dropout_rate),
            )
            self.meta_gate = nn.Parameter(torch.tensor(float(meta_gate_init)))
            logging.info(
                f"Meta feature context enabled: dim={self.meta_feature_dim}, "
                f"gate_init={float(meta_gate_init):.4f}"
            )
            if self.use_meta_query_conditioning:
                self.meta_query_proj = nn.Sequential(
                    nn.Linear(self.meta_feature_dim, d_model),
                    nn.LayerNorm(d_model),
                    nn.SiLU(),
                    nn.Dropout(dropout_rate),
                )
                self.meta_query_gate = nn.Parameter(torch.tensor(float(meta_query_gate_init)))
                logging.info(
                    "Early meta query conditioning enabled: "
                    f"gate_init={float(meta_query_gate_init):.4f}"
                )
    ...
    def _init_params(self) -> None:
        ...
        if self.use_meta_features:
            ...
            if self.use_meta_query_conditioning:
                for module in self.meta_query_proj:
                    if isinstance(module, nn.Linear):
                        nn.init.xavier_normal_(module.weight.data)
                        if module.bias is not None:
                            nn.init.zeros_(module.bias.data)
    ...
    def _init_structured_user_dense_modules(self, d_model: int) -> bool:
        """Initialize structure-aware user-dense projections when the expected
        fid layout is available in the runtime schema.
        """
        required_dense_fids = set(self._structured_user_dense_single_fids)
        for group in self._structured_user_dense_pair_groups:
            required_dense_fids.update(group)
        required_int_fids = {
            fid for group in self._structured_user_dense_pair_groups for fid in group
        }

        if not required_dense_fids.issubset(self._user_dense_fid_to_slice):
            missing = sorted(required_dense_fids.difference(self._user_dense_fid_to_slice))
            logging.info(
                "Structured user dense tokenizer disabled: "
                f"missing dense fids {missing}; falling back to monolithic user_dense token"
            )
            return False
        if not required_int_fids.issubset(self._user_int_fid_to_idx):
            missing = sorted(required_int_fids.difference(self._user_int_fid_to_idx))
            logging.info(
                "Structured user dense tokenizer disabled: "
                f"missing user-int fids {missing}; falling back to monolithic user_dense token"
            )
            return False

        self.user_dense_single_projs = nn.ModuleDict()
        self.user_dense_pair_projs = nn.ModuleDict()
        component_count = 0

        for fid in self._structured_user_dense_single_fids:
            _, length = self._user_dense_fid_to_slice[fid]
            self.user_dense_single_projs[str(fid)] = nn.Sequential(
        ...
        for group in self._structured_user_dense_pair_groups:
            dense_dim = sum(self._user_dense_fid_to_slice[fid][1] for fid in group)
            int_dim = len(group) * self.emb_dim
            self.user_dense_pair_projs[self._user_dense_group_key(group)] = nn.Sequential(
                nn.Linear(dense_dim + int_dim, d_model),
                nn.LayerNorm(d_model),
            )
            component_count += 1

        self.user_dense_fusion = nn.Sequential(
            nn.Linear(component_count * d_model, d_model),
            nn.LayerNorm(d_model),
        )
        return True

    def _user_dense_group_key(self, fids: Tuple[int, ...]) -> str:
        return "_".join(str(fid) for fid in fids)

    def _gather_user_dense_slice(
        self,
        ...
        for fid in feature_ids:
            offset, length = self._user_dense_fid_to_slice[fid]
            chunks.append(dense_feats[:, offset:offset + length])
        return torch.cat(chunks, dim=-1)

    def _embed_tokenizer_feature(
        self,
        int_feats: torch.Tensor,
        tokenizer: Union[GroupNSTokenizer, RankMixerNSTokenizer],
        feature_idx: int,
    ) -> torch.Tensor:
        """Embed a single feature using the tokenizer's shared embedding tables."""
        _, offset, length = tokenizer.feature_specs[feature_idx]
        emb_real_idx = tokenizer._emb_index[feature_idx]
        if emb_real_idx == -1:
            dtype = tokenizer.embs[0].weight.dtype if len(tokenizer.embs) > 0 else torch.float32
            return torch.zeros(
    ...
    def _build_user_dense_token(self, inputs: ModelInput) -> Optional[torch.Tensor]:
        """Build the user-dense token.

        When the analyzed fid layout is available, keep token count unchanged
        but construct the token from structured sub-blocks:
        - dense_61
        - dense_87
        - pair_62_66
        - pair_89_91
        Otherwise fall back to the original monolithic projection.
        """
        if not self.has_user_dense:
            return None

        if not self.use_structured_user_dense:
            return F.silu(self.user_dense_proj(inputs.user_dense_feats)).unsqueeze(1)

        component_tokens = []
        for fid in self._structured_user_dense_single_fids:
            dense_slice = self._gather_user_dense_slice(inputs.user_dense_feats, (fid,))
            component_tokens.append(F.silu(self.user_dense_single_projs[str(fid)](dense_slice)))

        for group in self._structured_user_dense_pair_groups:
            dense_slice = self._gather_user_dense_slice(inputs.user_dense_feats, group)
            int_embs = [
                self._embed_tokenizer_feature(
            ...
            pair_input = torch.cat([dense_slice] + int_embs, dim=-1)
            component_tokens.append(
                F.silu(self.user_dense_pair_projs[self._user_dense_group_key(group)](pair_input))
            )

        fused = torch.cat(component_tokens, dim=-1)
        return F.silu(self.user_dense_fusion(fused)).unsqueeze(1)

    def _build_ns_tokens(self, inputs: ModelInput) -> torch.Tensor:
        """Build non-sequence tokens for both train and inference paths."""
        user_ns = self.user_ns_tokenizer(inputs.user_int_feats)
        item_ns = self.item_ns_tokenizer(inputs.item_int_feats)

        ns_parts = [user_ns]
        user_dense_tok = self._build_user_dense_token(inputs)
    ...
    def _apply_meta_context(self, output: torch.Tensor, inputs: ModelInput) -> torch.Tensor:
        """Inject dense meta summaries without changing the token count."""
        if not self.use_meta_features:
            return output
        meta = inputs.meta_feats.float()
        return output + self.meta_gate * self.meta_proj(meta)

    def _build_meta_query_context(self, inputs: ModelInput) -> Optional[torch.Tensor]:
        """Project meta summaries into the query-generation path."""
        if not self.use_meta_query_conditioning:
            return None
        meta = inputs.meta_feats.float()
        return self.meta_query_gate * self.meta_query_proj(meta)
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 1. NS tokens: grouped sparse tokens + structured dense token(s)
        ...
        meta_query_context = self._build_meta_query_context(inputs)
        q_tokens_list = self.query_generator(
            ns_tokens, seq_tokens_list, seq_masks_list,
            context_token=meta_query_context,
        )
        ...
        output = self._apply_meta_context(output, inputs)
