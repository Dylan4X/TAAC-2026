...
# Evaluation stays on the baseline inference path: no flash-attn install,
# no runtime acceleration, always use SDPA.
os.environ["TAAC_ATTN_BACKEND"] = "sdpa"
...
_FALLBACK_MODEL_CFG = {
    ...
    'item_ns_tokens': 0,
    'meta_feature_dim': -1,
    'use_meta_query_conditioning': True,
    'meta_query_gate_init': 0.01,
}
...
def build_model(
    ...
    if int(model_cfg.get('meta_feature_dim', -1)) < 0:
        model_cfg['meta_feature_dim'] = dataset.meta_feature_dim
        logging.info(
            f"Resolved fallback meta_feature_dim from dataset: "
            f"{model_cfg['meta_feature_dim']}"
        )
    model = PCVRHyFormer(
        ...
        user_int_entries=list(dataset.user_int_schema.entries),
        user_dense_entries=list(dataset.user_dense_schema.entries),
